...
function generate_source_url_from_version {
    echo "https://github.com/llvm/llvm-project/releases/download/llvmorg-$1/cfe-$1.src.tar.xz"
}
...
if [[ $useSystemBinaries == [Yy] ]]; then
    ...
    clang_local_binaries=$(echo -n "$(find {,/usr}{,/local}/{,s}bin/clang-format{,-[0-9]*} 2>/dev/null)")
    for clang_bin in $clang_local_binaries
    do
        version=$(${clang_bin} --version | gsed 's/.*version \([^ -]*\).*/\1/')
...
do
    ...
    then
        ...
        mkdir "$version.src"
        wget "$source_url" --quiet -O - | tar "$(tar_flags "$source_url")" --strip-components=1 -C "$version.src" '*/docs/ClangFormatStyleOptions.rst'
